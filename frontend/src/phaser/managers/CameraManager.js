...
export class CameraManager {
  ...
  setupMiddleClickDrag() {
    this.scene.input.on("pointerdown", (pointer) => {
      ...
      if (
        typeof this.scene.canStartCameraDrag === "function" &&
        !this.scene.canStartCameraDrag(pointer)
      ) {
        return;
      }
  ...
  setupZoom() {
    this.scene.input.on("wheel", (pointer, gameObjects, deltaX, deltaY) => {
      const zoomDelta =
        deltaY > 0 ? -CAMERA_ZOOM.wheelStep : CAMERA_ZOOM.wheelStep;
  ...
  setupPinchZoom() {
    ...
    this.pinch.on("pinch", (pinch) => {
      ...
      this.setZoom(this.camera.zoom * pinch.scaleFactor, pinchCenter, {
        snap: false,
      });
    });
