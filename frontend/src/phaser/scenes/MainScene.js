...
export class MainScene extends Phaser.Scene {
  ...
  async loadConfigAndSetup() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load universe config: ${response.statusText}`,
        );
      }
    ...
    for (let i = 0; i < this.universeConfig.worldImages.length; i++) {
      ...
      this.load.image(`world_${i}`, basePath + imgConfig.path);
    }
  ...
  setupInputHandlers() {
    ...
    this.input.on("dragstart", (pointer, gameObject) => {
      ...
      if (!this.selectedBlocks.has(gameObject)) {
        if (
          this.pointerMultiSelectActive ||
          this.isMultiSelectModifierActive(pointer)
        ) {
    ...
    this.input.on("drag", (pointer, gameObject) => {
      if (gameObject instanceof Block) {
        ...
        if (this.blockDragSnapshot && this.selectedBlocks.has(gameObject)) {
          ...
          const deltaGridX =
            draggedGrid.gridX - this.blockDragSnapshot.originGridX;
          const deltaGridY =
            draggedGrid.gridY - this.blockDragSnapshot.originGridY;

          for (const block of this.selectedBlocks) {
            const startPosition = this.blockDragSnapshot.positions.get(
              block.blockKey,
            );
    ...
    this.input.on("dragend", (pointer, gameObject) => {
      if (gameObject instanceof Block) {
        ...
        for (const block of draggedBlocks) {
          const previousPosition = this.blockDragSnapshot?.positions.get(
            block.blockKey,
          ) ?? {
            gridX: block.gridX,
            gridY: block.gridY,
  ...
  loadBlockImages(blockImages, onComplete) {
    const toLoad = blockImages.filter(
      (img) =>
        !this.textures.exists(`block_${img.id}_${img.layer}_${img.rarity}`),
    );
  ...
  selectBlockForPlacement(blockData) {
    ...
    this.createPhantomBlock(
      `block_${blockData.id}_${blockData.layer}_${blockData.rarity}`,
    );
  }
  ...
  setSelectedBlocks(blocks, options = {}) {
    ...
    if (this.selectedBlocks.size > 0) {
      if (this.onBlockSelectedCallback) {
        this.onBlockSelectedCallback(
          this.selectedBlock?.blockKey,
  ...
  updateSelectionMarquee(pointer) {
    this.selectionCurrentWorldPoint = this.cameras.main.getWorldPoint(
      pointer.x,
      pointer.y,
    );
    this.drawSelectionMarquee();
  ...
  getSelectionRectangle() {
    ...
    const x = Math.min(
      this.selectionStartWorldPoint.x,
      this.selectionCurrentWorldPoint.x,
    );
    const y = Math.min(
      this.selectionStartWorldPoint.y,
      this.selectionCurrentWorldPoint.y,
    );
    const width = Math.abs(
      this.selectionCurrentWorldPoint.x - this.selectionStartWorldPoint.x,
    );
    const height = Math.abs(
      this.selectionCurrentWorldPoint.y - this.selectionStartWorldPoint.y,
    );
