...
class DailySelectionManager:
    ...
    def get_or_create_initial_blocks(universe_id: str, available_blocks: list) -> list:
        ...
        # Create new selection for this universe
        return DailySelectionManager.create_initial_blocks(
            universe_id, available_blocks
        )
    ...
    def create_daily_blocks(universe_id: str, available_blocks: list) -> list:
        ...
        indices = random.choices(
            range(len(available_blocks)), weights=weights, k=selected_count
        )

        selected_block_ids = [available_blocks[i]["id"] for i in indices]

        # Store in memory cache for current day
        DailySelectionManager._daily_blocks_by_universe[universe_id] = (
            selected_block_ids
        )
    ...
    def create_initial_blocks(universe_id: str, available_blocks: list) -> list:
        ...
        # Store in memory cache for current day
        DailySelectionManager._initial_blocks_by_universe[universe_id] = (
            selected_block_ids
        )
    ...
    def get_cache_stats() -> dict:
        ...
        return {
            ...
            "daily_selctions_universes": len(
                DailySelectionManager._daily_blocks_by_universe
            ),
            "initial_blocks_universes": len(
                DailySelectionManager._initial_blocks_by_universe
            ),
        }
