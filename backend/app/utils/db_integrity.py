...
class DatabaseIntegrity:
    ...
    def cleanup_orphaned_records(db: Session, dry_run: bool = False) -> dict:
        ...
        """
        stats = {"orphaned_worlds": 0, "orphaned_inventories": 0, "orphaned_likes": 0}

        # Find orphaned worlds
        orphaned_worlds = (
            db.query(World).filter(~World.user_id.in_(db.query(User.id))).all()
        )
        ...
        orphaned_inventories = (
            db.query(UserInventory)
            .filter(~UserInventory.user_id.in_(db.query(User.id)))
            .all()
        )
        ...
        # Find orphaned likes
        orphaned_likes = (
            db.query(Like).filter(~Like.user_id.in_(db.query(User.id))).all()
        )
