...
def list_universes(
    ...
    try:
        ...
        inventory = (
            db.query(UserInventory).filter(UserInventory.user_id == user_id).first()
        )

        if not inventory:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="User inventory not found"
            )

        # Distribute blocks for each universe first
        for universe in universes:
            try:
                BlockService.distribute_blocks_to_user(db, inventory.id, universe["id"])
            except Exception:
        ...
        total_available_blocks = (
            db.query(func.sum(InventoryBlock.quantity))
        ...
        for universe in universes:
            ...
            world = (
                db.query(World)
                .filter(World.user_id == user_id, World.universe_id == universe["id"])
                .first()
            )
            ...
            if world:
                placed_block_count = (
                    len(world.placed_blocks) if world.placed_blocks else 0
                )
...
def get_universe_blocks(universe_id: str):
    ...
    try:
        ...
        return {"universe_id": universe_id, "blocks": blocks, "count": len(blocks)}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
...
def enter_universe_auth(
    universe_id: str, authorization: str = Header(None), db: Session = Depends(get_db)
):
