...
def get_block_catalog(
    universe_id: str = Query(default=None), db: Session = Depends(get_db)
):
...
def get_daily_blocks(
    ...
    try:
        ...
        daily_block_ids = DailySelectionManager.get_or_create_daily_blocks(
            universe_id, available_blocks_config
        )

        # Filter to only daily blocks
        daily_blocks = [block for block in all_blocks if block.id in daily_block_ids]
