...
class UniverseService:
    ...
    UNIVERSES_BASE_PATH = (
        Path(__file__).parent.parent.parent.parent / "public" / "univers"
    )
                    ...
                    universes.append(
                        {
                            "id": universe_id,
                            "name": config.get(
                                "name", universe_id.replace("_", " ").title()
                            ),
                            "config": config,
    ...
    def initialize_user_in_universe(
        db: Session, user_id: str, universe_id: str
    ) -> dict:
        ...
        existing_world = (
            db.query(World)
            .filter(World.user_id == user_id, World.universe_id == universe_id)
            .first()
        )

        if existing_world:
            return {"world_id": existing_world.id, "is_new": False}
        ...
        all_blocks = (
            db.query(BlockCatalog).filter(BlockCatalog.universe_id == universe_id).all()
        )
        ...
        return {"world_id": world.id, "universe_id": universe_id, "is_new": True}
