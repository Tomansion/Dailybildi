...
class BlockService:
    ...
    def distribute_blocks_to_user(
        db: Session, inventory_id: str, universe_id: str
    ) -> dict:
        """
        ...
        if not all_blocks:
            raise ValueError(
                f"No blocks found for universe '{universe_id}'. Check config.json in public/univers/{universe_id}/"
            )
        ...
        # Get inventory
        inventory = (
            db.query(UserInventory).filter(UserInventory.id == inventory_id).first()
        )
        if not inventory:
        ...
        if last_distribution_date is None:
            ...
            block_ids = DailySelectionManager.get_or_create_initial_blocks(
                universe_id, [{"id": b.id, "rarity": b.rarity} for b in all_blocks]
            )
            ...
            added_count = InventoryService.add_blocks_batch(
                db, inventory.id, dict(block_id_counts)
            )
        ...
        current_count = (
            ...
            .filter(InventoryBlock.inventory_id == inventory_id)
        ...
        new_target = min(
            current_count + blocks_to_add_per_day, settings.INITIAL_BLOCKS_COUNT
        )
        ...
        if blocks_to_add > 0:
            ...
            distributed = BlockService.select_and_add_blocks(
                db, inventory_id, all_blocks, blocks_to_add
            )
        else:
    ...
    def select_and_add_blocks(
        db: Session, inventory_id: str, blocks: list, count: int
    ) -> int:
        ...
        selected_indices = random.choices(range(len(blocks)), weights=weights, k=count)
        ...
        return InventoryService.add_blocks_batch(
            db, inventory_id, dict(block_id_counts)
        )
    ...
    def _ensure_blocks_in_catalog(db: Session, universe_id: str, blocks: list) -> None:
        ...
        for block in blocks:
            ...
            existing = (
                ...
                .filter(
                    BlockCatalog.id == block.id, BlockCatalog.universe_id == universe_id
                )
                .first()
            )
