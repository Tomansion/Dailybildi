...
class TestDuplicatePlacement:
    ...
    def test_cannot_place_block_without_inventory(
        self, db, test_user, test_inventory, test_blocks_catalog
    ):
        """
        ...
        world = World(
            id="world-1", user_id=test_user.id, universe_id=settings.UNIVERSE_ID
        )
        db.add(world)
        db.commit()
        db.refresh(world)
    ...
    def test_cannot_place_block_twice_with_quantity_one(
        self, db, test_user, test_inventory, test_blocks_catalog
    ):
        """
    ...
    def test_can_place_multiple_blocks_if_quantity_allows(
        self, db, test_user, test_inventory, test_blocks_catalog
    ):
        """
        ...
        # Create a world
        world = World(
            id="world-2", user_id=test_user.id, universe_id=settings.UNIVERSE_ID
        )
        db.add(world)
        db.commit()
        db.refresh(world)
        ...
        world_placed = (
            db.query(PlacedBlock).filter(PlacedBlock.world_id == world.id).all()
        )
        assert len(world_placed) == 2, "Should have 2 placed blocks"

    def test_placement_decrements_inventory(
        self, db, test_user, test_inventory, test_blocks_catalog
    ):
        """
        ...
        # Create world
        world = World(
            id="world-3", user_id=test_user.id, universe_id=settings.UNIVERSE_ID
        )
        db.add(world)
        db.commit()
