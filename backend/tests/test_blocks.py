...
class TestBlockDistributionLogic:
    ...
    def test_first_distribution_gives_30_blocks(self, db, test_inventory):
        ...
        result = BlockService.distribute_blocks_to_user(
            db, test_inventory.id, "test_universe"
        )
    ...
    def test_same_day_no_distribution(self, db, test_inventory):
        ...
        # First distribution
        BlockService.distribute_blocks_to_user(db, test_inventory.id, "test_universe")
    ...
    def test_inventory_decreases_on_block_removal(self, db, test_inventory):
        ...
        InventoryService.remove_block_from_inventory(
            db, test_inventory.user_id, block_to_remove.block_catalog_id, quantity=1
        )
...
class TestInventory:
    ...
    def test_add_same_block_twice_increments(
        self, db, test_inventory, test_blocks_catalog
    ):
    ...
    def test_remove_block_decreases_quantity(
        self, db, test_inventory, test_blocks_catalog
    ):
        ...
        # Remove 2
        InventoryService.remove_block_from_inventory(
            db, test_inventory.user_id, first_block.id, quantity=2
        )

        db.refresh(test_inventory)
        assert test_inventory.blocks[0].quantity == 3

    def test_remove_block_deletes_when_empty(
        self, db, test_inventory, test_blocks_catalog
    ):
        ...
        InventoryService.remove_block_from_inventory(
            db, test_inventory.user_id, first_block.id, quantity=1
        )
