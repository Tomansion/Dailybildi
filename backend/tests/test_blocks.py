...
class TestBlockDistributionLogic:
    ...
    def test_clearing_on_next_day_with_low_inventory(self, db, test_inventory):
        """
        Next day with ≤10 blocks: clear old inventory, add 10 new
        """
        from datetime import date, timedelta
        
        # First distribution: 30 blocks
        BlockService.distribute_blocks_to_user(
            db,
            test_inventory.id,
            "test_universe"
        ...
        # Manually reduce to 8 blocks (simulating user placed many)
        db.refresh(test_inventory)
        total = sum(b.quantity for b in test_inventory.blocks)
        to_remove = total - 8
        
        for block in test_inventory.blocks:
            if to_remove <= 0:
                break
            if block.quantity >= to_remove:
                block.quantity -= to_remove
                to_remove = 0
            else:
                to_remove -= block.quantity
                block.quantity = 0
        db.commit()
        
        db.refresh(test_inventory)
        before_next_day = sum(b.quantity for b in test_inventory.blocks)
        assert before_next_day == 8, f"Setup failed: {before_next_day} != 8"
        
        # Simulate yesterday by updating distribution date
        yesterday = str(date.today() - timedelta(days=1))
        new_dists = test_inventory.last_distributions or {}
        new_dists["test_universe"] = yesterday
        test_inventory.last_distributions = new_dists
        flag_modified(test_inventory, "last_distributions")
        db.commit()
        db.refresh(test_inventory)
        
        # Distribute on "today" (next day from yesterday)
        result = BlockService.distribute_blocks_to_user(
            db,
            test_inventory.id,
            "test_universe"
        ...
        assert result["action"] == "daily", f"Expected 'daily', got '{result['action']}''"
        assert result["cleared_old_inventory"] is True, f"Should clear ≤10, had {before_next_day}"
        assert result["previous_count"] == 8
        assert result["blocks_added"] == 10
        
        db.refresh(test_inventory)
        after_next_day = sum(b.quantity for b in test_inventory.blocks)
        # Should be cleared (0) + 10 new = 10
        assert after_next_day == 10, f"Expected 10, got {after_next_day}"
    
    def test_keeping_inventory_on_next_day_with_high_inventory(self, db, test_inventory):
        """
        Next day with >10 blocks: keep all, add 10 new
        """
        from datetime import date, timedelta
        ...
        # Reduce to 15 blocks (simulating user placed 15)
        db.refresh(test_inventory)
        total = sum(b.quantity for b in test_inventory.blocks)
        to_remove = total - 15
        ...
        assert result["action"] == "daily", f"Expected 'daily', got '{result['action']}''"
        assert result["cleared_old_inventory"] is False, "Should NOT clear >10 blocks"
        assert result["blocks_added"] == 10
        
        db.refresh(test_inventory)
        after_next_day = sum(b.quantity for b in test_inventory.blocks)
        # Should be 15 + 10 = 25
        assert after_next_day == 25, f"Expected 25, got {after_next_day}"
